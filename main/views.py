...
def ClientPage(request):
    ...
    # Statistikalar
    today = timezone.now().date()
    active_clients = clients.filter(created__date__gte=today - timedelta(days=30))
    today_clients = clients.filter(created__date=today).count()
        ...
        try:
            ...
            while True:
                ...
                if member_birth_date:
                    try:
                        birth_date_parsed = _parse_contract_date(member_birth_date)
                    except ValueError:
                        pass
...
def ContractCreatePDF(request, id):
    contract = get_object_or_404(ConsultingContract, pk=id)
    legacy_contract = contract.as_legacy_contract()
    ...
    initial_due_date = contract.contract_date + timedelta(days=contract.initial_payment_due_days or 0)
    if contract.initial_payment_amount:
        current_balance -= contract.initial_payment_amount
        pay_list.append({
            "day": initial_due_date.day,
            "month": month_names_uz[initial_due_date.month],
            "year": initial_due_date.year,
            "payment": contract.initial_payment_amount,
            "remaining": max(current_balance, 0),
        })

    second_due_date = initial_due_date + timedelta(days=contract.post_interview_due_days or 0)
    if contract.post_interview_payment_amount:
        current_balance -= contract.post_interview_payment_amount
        pay_list.append({
            "day": second_due_date.day,
            "month": month_names_uz[second_due_date.month],
            "year": second_due_date.year,
            "payment": contract.post_interview_payment_amount,
            "remaining": max(current_balance, 0),
        })
    ...
    html_content = render_to_string(
        "shart.html",
        {
            ...
            "price_text": number_to_words_uz(int(total_price)),
            "pay_text": number_to_words_uz(int(initial_paid)),
            "foiz": foiz_formatted,
            "dr": qisqartirish(contract.client_full_name),
            "total_price": 0,
            "down_payment": 0,
            "remaining_balance": remaining_balance,
            "down_payment_percentage": foiz,
            "pay_list": pay_list,
        },
    )
